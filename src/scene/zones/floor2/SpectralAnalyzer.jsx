...
export function SpectralAnalyzer({ onClick, active, band = 1 }) {
  ...
  return (
    ...
    >
      ...
      {/* Inset bezel */}
      <mesh position={[0, PANEL_CY, PANEL_D / 2 - 0.01]}>
        <boxGeometry args={[PANEL_W * 0.92, PANEL_H * 0.84, 0.04]} />
        <Wire color={z.color} />
      </mesh>
      ...
      <mesh position={[0, PANEL_CY + PANEL_H / 2 + 0.04, 0]}>
        <boxGeometry args={[PANEL_W * 0.96, 0.08, PANEL_D * 0.6]} />
        <Wire color={z.color} />
      </mesh>
      ...
      <group position={[PANEL_W / 2 + 0.005, PANEL_CY - 0.25, 0]} rotation={[0, Math.PI / 2, 0]}>
        {[0, 0.08, 0.16, 0.24, 0.32, 0.40, 0.48].map((y, i) => (
          <mesh key={i} position={[0, y, 0]}>
            ...
            <meshBasicMaterial color={i === 6 ? '#c8210a' : i === 5 ? '#e8501a' : i >= 3 ? '#ffd23a' : z.color} toneMapped={false} />
          </mesh>
        ))}
      </group>
